...
export default function FavoriteTracks() {
  ...
  // Tracks map that will represent a track id and a bool whether the user owns it or not
  ...
  */
  const retrieveUserTracks = async () => {
    await getUserOwnedTracks(user, ownedTracks);
  };
  retrieveUserTracks();
  ...
  */
  const handleTrackSelected = (track, selected) => {
    ownedTracks.set(parseInt(track), selected);
  };
  ...
  */
  const getTracks = () => {
    ...
    Object.values(Tracks).forEach((trackInfo) => {
      // Set the owned tracks to false
      ownedTracks.set(trackInfo.id, false);
